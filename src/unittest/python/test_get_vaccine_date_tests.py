...
class TestGetVaccineDate(TestCase):
    ...
    @freeze_time("2022-03-08")
    def test_get_vaccine_date_no_ok_date_invalid(self):
        """not valid test"""
        file_test = JSON_FILES_RF2_PATH + "test_ok.json"
        my_manager = VaccineManager()
        file_store_date = AppointmentsJsonStore()
        ...
        with self.assertRaises(VaccineManagementException) as c_m:
            my_manager.get_vaccine_date(file_test, date)
        self.assertEqual(c_m.exception.message, "Appointment date is not valid")
    ...
    @freeze_time("2022-03-08")
    def test_get_vaccine_date_no_ok_date_invalid2(self):
        """not valid test 2"""
        file_test = JSON_FILES_RF2_PATH + "test_ok.json"
        my_manager = VaccineManager()
        file_store_date = AppointmentsJsonStore()
    ...
    def test_get_vaccine_date_no_ok_date_invalid2(self):
        ...
        with self.assertRaises(VaccineManagementException) as c_m:
            my_manager.get_vaccine_date(file_test, date)
        self.assertEqual(c_m.exception.message, "date format is not valid")
